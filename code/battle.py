...
class Battle:
    ...
    def setup(self):
        for entity, monster in self.monster_data.items():
            ...
            #remove the opponent monster data
    ...
    def opponent_attack(self):
        ...
        self.current_monster.active_attack(random_target, ability)
    ...
    def input(self):
        if (self.selection_mode and self.current_monster and 
        self.player_sprites.has(self.current_monster)):
            ...
            match self.selection_mode:
                ...
                case 'target':
                    # When selecting the target, limit based on player or opponent
            ...
            if keys[pygame.K_SPACE]:
                if self.selection_mode == 'switch':
                    ...
                    for monster_sprite in self.player_sprites.sprites() + self.opponent_sprites.sprites():
                        monster_sprite.set_highlight(False)

                    # Get the correct sprite group (player or opponent)
                    ...
                    sprites.sort(key=lambda s: s.pos_index)  # Sort by position index

                    # Make sure the target index is within bounds
                    if self.indexes['target'] >= len(sprites):
                        self.indexes['target'] = len(sprites) - 1  # Correct the index if it's out of bounds
    ...
    def apply_attack(self,target_sprite, attack, amount_damage):
        ...
        target_sprite.monster.stat_limiter()
        self.check_dealth()
    ...
    def check_dealth(self):
        for monster_sprite in self.opponent_sprites.sprites() + self.player_sprites.sprites():
            if monster_sprite.monster.health <= 0:
                if self.player_sprites in monster_sprite.groups():
                    active_monster = [(monster_sprite.index, monster_sprite.monster) for monster_sprite in self.player_sprites.sprites()]
                    available_monster = [(index, monster) for index, monster in self.monster_data['player'].items() if monster.health > 0 and (index, monster) not in active_monster]
                    if available_monster:
                        #new_monster_data = [(monster, index, monster_sprite.pos_index, 'player') for index, monster in active_monster ][0]
                        index, monster = available_monster[0]
    ...
    def draw_ui(self):
        if self.current_monster:
            if self.selection_mode == "general":
                ...
                # Select the correct group based on selection_side
                sprite_group = self.opponent_sprites if self.selection_side == 'opponent' else self.player_sprites
                sprites = list(sprite_group.sprites())
                sprites.sort(key=lambda s: s.pos_index)

                # Defensive: Ensure index is valid
                if sprites:
                    target_index = min(self.indexes['target'], len(sprites) - 1)
                    selected_target = sprites[target_index]

                    # Highlight the selected target
                    selected_target.set_highlight(True)
    ...
    def draw_attacks(self):
        ...
        v_offset = 0 if self.indexes['attack'] < visible_attacks else -(self.indexes['attack'] - visible_attacks + 1) * item_height
    ...
    def draw_switch(self):
        ...
        #monster
        active_monster = [(monster_sprite.index, monster_sprite.monster) for monster_sprite in self.player_sprites]
        self.available_monsters = {
            index: monster for index, monster in self.monster_data['player'].items() if (index, monster) not in active_monster and monster.health > 0
        }
