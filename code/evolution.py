...
class Evolution:
    def __init__(self, frames, start_monster, end_monster, font, end_evolution, star_animation_frames):
        ...
        self.timers = {
            'start': Timer(500, autostart = True),
            'end': Timer(2250, func = end_evolution)
        }

        # # star animation
        self.star_animation_frames = [pygame.transform.scale2x(frame) for frame in star_animation_frames]
        self.frame_index = 0
        ...
        self.tint_amount, self.tint_speed = 0, 150
    ...
    def display_stars(self, dt):
        ...
        if self.frame_index < len(self.star_animation_frames):
            frame = self.star_animation_frames[int(self.frame_index)]
