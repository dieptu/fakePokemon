...
class MonsterIndex:
    ...
    def display_main(self, dt):
        ...
        pygame.draw.rect(self.display_surface, COLORS['dark'], rect, 0, 12 ,0 ,12)
        ...
        healthbar_rect = pygame.FRect((0,0), (bar_data['width'],bar_data['heigh'])).move_to(midtop = (bar_data['left_side'], bar_data['top']))
        draw_bar(
            self.display_surface, healthbar_rect, monster.health, monster.get_stat('max_health'), COLORS['red'], COLORS['black'], 2
        )
        ...
        energybar_rect = pygame.FRect((0,0), (bar_data['width'],bar_data['heigh'])).move_to(midtop = (bar_data['right_side'], bar_data['top']))
        draw_bar(
            self.display_surface, energybar_rect, monster.energy, monster.get_stat('max_energy'), COLORS['blue'], COLORS['black'], 2
        )
        ...
        stats_text_surf = self.fonts['regular'].render('Stat', False, COLORS['white'])
        ...
        #pygame.draw.rect(self.display_surface, 'red', stats_rect)

        monster_stats = monster.get_stats()
        stat_heights = stats_rect.height / len(monster_stats)
        for index, (stat, value) in enumerate(monster_stats.items()):
            single_stat_rect = pygame.FRect(stats_rect.left, stats_rect.top + index *stat_heights , stats_rect.width, stat_heights)
            #icon
